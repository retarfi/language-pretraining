...
class LineByLineTextDataset(Dataset):
    ...
    def __init__(self, tokenizer: PreTrainedTokenizer, file_path: str, block_size: int):
        ...
        if os.path.isfile(file_path):
            examples = batch_encoding(file_path, tokenizer, block_size)
            self.examples = [{"input_ids": torch.tensor(e, dtype=torch.long)} for e in examples]
        else:
            with Pool(os.cpu_count()) as pool:
                mp_task = [pool.apply_async(batch_encoding, (file_path_i, tokenizer, block_size)) for file_path_i in glob.glob(os.path.join(file_path, '*.txt'))]
                examples = [f.get() for f in mp_task]
            self.examples = [{"input_ids": torch.tensor(e, dtype=torch.long)} for doc in examples for e in doc]
